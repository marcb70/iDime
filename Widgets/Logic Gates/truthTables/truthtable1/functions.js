...
function updateTable()
{
	...
	if(circleArray.length<16){//if there are less than 16 circles in the main array, there is at least one blank circle. increase error count
		errors++;
	}
	else{

		for(var i = 0; i < circle3Array.length; i++){
			var color = circle3Array[i].getFill();//get the color of the element in the array
			var object = circle3Array[i].getId();//get the id of the element in the array
			//console.log("for loop 1 Object id and color: " + object + ", " + color);
			if(circle3Array.length<4){//if this array is less than 4, there is at least one blank circle in this column
				errors++;
			}
			else{
	...
	if(errors==0){
		...
		if(circle3Array.length<4||circle4Array.length<4||circle5Array.length<4||circle6Array.length<4||circleArray.length<16)
		{
