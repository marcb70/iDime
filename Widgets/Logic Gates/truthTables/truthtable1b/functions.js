...
function updateTable()
{
	...
	if(circleArray.length<16){//if the main array contains less than 16 circles, there is at least one blank circle in the table. increase error count.
		errors++;
	}
	else{

		for(var i = 0; i < circle3Array.length; i++){
			var color = circle3Array[i].getFill();//get color of the element in the array
			var object = circle3Array[i].getId();//get id of the element in the array
	...
	for(var i = 0; i < circle7Array.length; i++){
			var color = circle7Array[i].getFill();
			var object = circle7Array[i].getId();
			//console.log("for loop 1 Object id and color: " + object + ", " + color);
			if(circle7Array.length<4){
				errors++;
			}
			else{
