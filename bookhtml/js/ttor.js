...
function ttorCall(){
	...
	/*creates an event each time a circle is clicked or tapped. Calls upon ttorsetColor() function in functions.js
	*for devices with a mouse device, the click function is used. For touch devices such as mobile and tablet devices, the tap function is used.*/
	...
	ttoraddToArray to add the object to its corresponding array and draws the layer to ensure color change.*/
	function ttorsetColor(object)
	{
		...
		ttoraddToArray(object);//add the object with it's id and color to an array
	...
	function ttoraddToArray(object){
		if(!ttorcircleArray.contains(object)){
				ttorcircleArray.push(object);
		}
	...
	function ttorupdateTable()
	{
		...
		if(ttorcircleArray.length<4){
			errors++;
		}
		else{

			for(var i = 0; i < ttorcircleArray.length; i++){
				var color = ttorcircleArray[i].getFill();
				var object = ttorcircleArray[i].getId();
	...
	var ttorhtml = '<div id = "ttorMain" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"> <!-- --> <div id="directions" class = "modal-header"> <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button> <h3 id="header">Build the truth table for the OR operator: </h3> <h4>P + Q</h4> </div> <!--The main modal body contains only the exercise.--> <div id="ttor" class = "modal-body"></div> <div id = "buttonDiv" class="modal-footer"> <!--This button allows the user to close the modal and return to reading the material. Work will not be saved.--> <button class="btn" data-dismiss="modal" aria-hidden="true">Close</button> <!--This button checks the user\'s work. Calls "ttorupdateTable" function. ttorupdateTable() is found in functions.js. --> <input type = "submit" class="btn btn-primary" value = "check it!" id = "ttorsubmit" onclick = "ttorupdateTable();" /> </div> </div>';
	function writettor(){
		$('#widgethtml').html(ttorhtml);
		ttorCall();
	}
