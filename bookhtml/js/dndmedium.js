function dndmediumloadImages(sources, callback) {
  var images = {};
  var loadedImages = 0;
  var numImages = 0;
  // get num of sources
...
function dndmediumdraw(images){
   ...
   var QtoDrop3 = new Kinetic.Line({
    ...
    notBox.on("dragend", function(){
      dndmediumcheckDrop(notBox);
    });
    andBox.on("dragend", function(){
      dndmediumcheckDrop(andBox);
    });
    orBox.on("dragend", function(){
      dndmediumcheckDrop(orBox);
    });

  function dndmediumresetXY()
  {
  ...
  function dndmediumdoDrop(object, dropZone)
  {
  ...
  dndmediumclear.onclick=function dndmediumclear(){
  ...
  mhelp.onclick = function dndmediumhelpInfo(){
  ...
  mcheck.onclick=function dndmediumcheckGates(){
    var error = 0;
...
var dndmediumhtml = '<div id = "dndmediumMain" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"><div id="directions" class = "modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button><h3 id="header">Drag and drop the logic gates into the correct boxes to match this statement: </h3><h4>(P * Q) + (Q * ~Q) + (P * ~Q)</h4></div><div id="dndmedium" class = "modal-body"></div><div id = "buttonDiv" class="modal-footer"><button class="btn" data-dismiss="modal" aria-hidden="true">Close</button><input type = "submit" class="btn btn-primary" value = "check it!" id = "mcheck" onclick = "dndmediumcheckGates();" /><input type = "submit" class="btn btn-danger" value = "Help!!" id = "mhelp" onclick = "dndmediumhelpInfo();" /><input type = "submit" class="btn btn-warning" value = "Clear it!" id = "dndmediumclear" onclick = "dndmediumclear();" /></div></div></div>';
    function writedndmed(){
      $('#widgethtml').html(dndmediumhtml);

       dndmediumloadImages(sources, function(images) {
        dndmediumdraw(images);
      });
      $('#dndmediumMain').css({
