	var tt2dcirleArray = new Array();
	var tt2dcirle4Array = new Array();
	var tt2dcirle5Array = new Array();
	var tt2dcirle6Array = new Array();
	var tt2dtrashArray = new Array();
...
function tt2dCall(){
	...
	*This file uses kineticJS to create each object.*/
	...
	tableShapes.add(RimpliesP);
	tableShapes.add(QimpliesR);
	...
	/*creates an event each time a circle is clicked or tapped. Calls upon tt2dsetColor() function in functions.js
	*for devices with a mouse device, the click function is used. For touch devices such as mobile and tablet devices, the tap function is used.*/
	...
	function tt2dclearColumn(object){
		//console.log(object);
	...
	tt2daddToArray to add the object to its corresponding array and draws the layer to ensure color change.*/
	function tt2dsetColor(object)
	{
		...
		tt2daddToArray(object);//add the object with it's id and color to an array
		tableShapes.draw();//draw the layer to make the color change. 
	}

	//this function serves to add an object to its correct array.
	function tt2daddToArray(object){
		//console.log("tt2daddToArray information: " + object.getFill() + ", " + object.getId());
		if(!tt2dcirleArray.contains(object)){
				tt2dcirleArray.push(object);
		}
		var objectId = object.getId();

		if(objectId=="4x1"||objectId=="4x2"||objectId=="4x3"||objectId=="4x4"||objectId=="4x5"||objectId=="4x6"||objectId=="4x7"||objectId=="4x8"){
			if(!tt2dcirle4Array.contains(object)){
				//console.log("pushing to tt2dcirle4Array" + objectId + " " + object.getFill());
				tt2dcirle4Array.push(object);
			}
		}
		else if(objectId=="5x1"||objectId=="5x2"||objectId=="5x3"||objectId=="5x4"||objectId=="5x5"||objectId=="5x6"||objectId=="5x7"||objectId=="5x8"){
			if(!tt2dcirle5Array.contains(object)){
				//console.log("pushing to tt2dcirle5Array" + objectId + " " + object.getFill());
				tt2dcirle5Array.push(object);
			}
		}
		else if(objectId=="6x1"||objectId=="6x2"||objectId=="6x3"||objectId=="6x4"||objectId=="6x5"||objectId=="6x6"||objectId=="6x7"||objectId=="6x8"){
			if(!tt2dcirle6Array.contains(object)){
				//console.log("pushing to tt2dcirle6Array" + objectId + " " + object.getFill());
				tt2dcirle6Array.push(object);
			}
		}
		else{
			//console.log("pushing to tt2dtrashArray" + objectId + " " + object.getFill());
			tt2dtrashArray.push(object);
		}
...
function tt2dupdateTable()
	{
		...
		if(tt2dcirleArray.length<24){
			errors++;
		}
		else{

			for(var j=0; j<tt2dcirle4Array.length; j++){
				var color1 = tt2dcirle4Array[j].getFill();
				var object1 = tt2dcirle4Array[j].getId();
				//console.log("for loop 2 Object id and color: " + object1 + ", " + color1);
				if(tt2dcirle4Array.length<8){
					errors++;
				}
				else{
					if(!((object1 == "4x1" && color1 == "green")||(object1 == "4x2" && color1 == "green")
			 			|| (object1 == "4x3" && color1 == "green") ||  (object1 == "4x4" && color1=="green")||
			 			(object1 == "4x5" && color1 == "red")||(object1 == "4x6" && color1 == "red")
			 			|| (object1 == "4x7" && color1 == "green") ||  (object1 == "4x8" && color1=="green"))){

						var temp1 = "The R -> P column has errors.";
			...
			for(var k=0; k<tt2dcirle5Array.length; k++){
				var color2 = tt2dcirle5Array[k].getFill();
				var object2 = tt2dcirle5Array[k].getId();
				//console.log("for loop 3 Object id and color: " + object2 + ", " + color2);
				if(tt2dcirle5Array.length<8){
					errors++;
				}
				else{
					if(!((object2 == "5x1" && color2 == "green")||(object2 == "5x2" && color2 == "red")
						...
						var temp2="The ~Q -> ~R column has errors.";
			...
			for(var l=0; l<tt2dcirle6Array.length; l++){
				var color3 = tt2dcirle6Array[l].getFill();
				var object3 = tt2dcirle6Array[l].getId();
				//console.log("for loop 3 Object id and color: " + object3 + ", " + color3);
				if(tt2dcirle6Array.length<8){
					errors++;
				}
				else{
					if(!((object3 == "6x1" && color3 == "green")||(object3 == "6x2" && color3 == "red")
			 			|| (object3 == "6x3" && color3 == "green") ||  (object3 == "6x4" && color3=="green")||
			 			(object3 == "6x5" && color3 == "red")||(object3 == "6x6" && color3 == "red")
		...
		if(errors==0){
			...
			if(tt2dcirle4Array.length<8||tt2dcirle5Array.length<8||tt2dcirle6Array.length<8||tt2dcirleArray.length<24)
			{
	...
	var tt2dhtml = '<div id = "tt2dMain" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"> <!-- --> <div id="directions" class = "modal-header"> <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button> <h3 id="header">P, Q and R have been filled in for you. Now, build the truth table for this statement: </h3> <h4>(R -> P) * (~Q -> ~R)</h4> </div> <!--The main modal body contains only the exercise.--> <div id="tt2d" class = "modal-body"></div> <div id = "buttonDiv" class="modal-footer"> <!--This button allows the user to close the modal and return to reading the material. Work will not be saved.--> <button class="btn" data-dismiss="modal" aria-hidden="true">Close</button> <!--This button checks the user\'s work. Calls "tt2dupdateTable" function. tt2dupdateTable() is found in functions.js. --> <input type = "submit" class="btn btn-primary" value = "check it!" id = "tt2dsubmit" onclick = "tt2dupdateTable();" /> <!--This button gives the user a hint Calls "hint" function. --> <input type = "submit" class="btn btn-danger" value = "Give me a Hint!" id = "tt2dhintme" onclick = "tt2dhint();" /> </div> </div>';
	function writett2d(){
		$('#widgethtml').html(tt2dhtml);
			tt2dCall();
