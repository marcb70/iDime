//function dndEasyCall(){
function dndeasyloadImages(sources, callback) {
        var images = {};
        var loadedImages = 0;
        var numImages = 0;
        // get num of sources
      ...
      // dndeasyloadImages(sources, function(images) {
      //   dndeasydraw(images);
      // });
        //draws stage with images 
      function dndeasydraw(images){
       ...
       var QtoDrop2 = new Kinetic.Line({
        ...
        notBox.on("dragend", function(){
          dndeasycheckDrop(notBox);
        });
        andBox.on("dragend", function(){
          dndeasycheckDrop(andBox);
        });
        orBox.on("dragend", function(){
          dndeasycheckDrop(orBox);
        });
        
        function dndeasyresetXY(){
        ...
        function dndeasycheckDrop(object)
        {
          ...
          else if((object.getPosition().x >= 550 && object.getPosition().x <= 650) && (object.getPosition().y >= 200 && object.getPosition().y <= 300))
          {
            ...
            dndeasydoDrop(object, drop4);
          }
          else
          {
            dndeasyresetXY(object);  
          }
          layer.draw()
        ...
        function dndeasydoDrop(object, dropZone)
        {
        ...
        dndeasyclear.onclick = function dndeasyclear(){
        ...
        ehelp.onclick = function dndeasyhelpInfo(){
        ...
        echeck.onclick = function dndeasycheckGates(){
          var error = 0;
  ...
  var dndeasyhtml = '<div id = "dndeasyMain" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"><div id="directions" class = "modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button><h3 id="header">Drag and drop the logic gates into the correct boxes to match this statement: </h3><h4>(P * Q) + ~(P + Q)</h4></div><div id="dndeasy" class = "modal-body"></div><div id = "buttonDiv" class="modal-footer"><button class="btn" data-dismiss="modal" aria-hidden="true">Close</button><input type = "submit" class="btn btn-primary" value = "check it!" id = "echeck" onclick = "dndeasycheckGates();" /><input type = "submit" class="btn btn-danger" value = "Help!!" id = "ehelp" onclick = "dndeasyhelpInfo();" /><input type = "submit" class="btn btn-warning" value = "Clear it!" id = "dndeasyclear" onclick = "dndeasyclear();" /></div></div>'
  function writedndeasy(){
      $('#widgethtml').html(dndeasyhtml);
      //dndEasyCall();
      dndeasyloadImages(sources, function(images) {
        dndeasydraw(images);
      });
