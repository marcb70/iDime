...
function ttxorCall(){
  ...
  /*creates an event each time a circle is clicked or tapped. Calls upon ttxorsetColor() function in functions.js
  *for devices with a mouse device, the click function is used. For touch devices such as mobile and tablet devices, the tap function is used.*/
  ...
  ttxoraddToArray to add the object to its corresponding array and draws the layer to ensure color change.*/
  function ttxorsetColor(object)
  {
  	...
  	ttxoraddToArray(object);//add the object with it's id and color to an array
  ...
  function ttxoraddToArray(object){
  	if(!ttxorcircleArray.contains(object)){
  			ttxorcircleArray.push(object);
  	}
  ...
  function ttxorupdateTable()
  {
    ...
    if(ttxorcircleArray.length<4){
      errors++;
    }
    else{

      for(var i = 0; i < ttxorcircleArray.length; i++){
        var color = ttxorcircleArray[i].getFill();
        var object = ttxorcircleArray[i].getId();
  ...
  var ttxorhtml = '<div id = "ttxorMain" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"> <!-- --> <div id="directions" class = "modal-header"> <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button> <h3 id="header">Build the truth table for the EXCLUSIVE OR (XOR) operator: </h3> <h4>P # Q</h4> </div> <!--The main modal body contains only the exercise.--> <div id="ttxor" class = "modal-body"></div> <div id = "buttonDiv" class="modal-footer"> <!--This button allows the user to close the modal and return to reading the material. Work will not be saved.--> <button class="btn" data-dismiss="modal" aria-hidden="true">Close</button> <!--This button checks the user\'s work. Calls "ttxorupdateTable" function. ttxorupdateTable() is found in functions.js. --> <input type = "submit" class="btn btn-primary" value = "check it!" id = "ttxorsubmit" onclick = "ttxorupdateTable();" /> </div> </div>';
  function writettxor(){
  	$('#widgethtml').html(ttxorhtml);
  	ttxorCall();
  }