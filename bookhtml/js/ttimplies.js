ttimpliescircleArray = new Array();

function ttimpliesCall(){
	...
	/*creates an event each time a circle is clicked or tapped. Calls upon ttimpliessetColor() function in functions.js
	*for devices with a mouse device, the click function is used. For touch devices such as mobile and tablet devices, the tap function is used.*/
	circle3x1.on("click tap", function(){
		ttimpliessetColor(circle3x1);
	})

	circle3x2.on("click tap", function(){
		ttimpliessetColor(circle3x2);
	})

	circle3x3.on("click tap", function(){
		ttimpliessetColor(circle3x3);
	})

	circle3x4.on("click tap", function(){
		ttimpliessetColor(circle3x4);
	})
	...
	ttimpliesaddToArray to add the object to its corresponding array and draws the layer to ensure color change.*/
	function ttimpliessetColor(object)
	{
		...
		ttimpliesaddToArray(object);//add the object with it's id and color to an array
	...
	function ttimpliesaddToArray(object){
		if(!ttimpliescircleArray.contains(object)){
				ttimpliescircleArray.push(object);
		}
	...
	function ttimpliesupdateTable()
	{
		...
		if(ttimpliescircleArray.length<4){
			errors++;
		}
		else{

			for(var i = 0; i < ttimpliescircleArray.length; i++){
				var color = ttimpliescircleArray[i].getFill();
				var object = ttimpliescircleArray[i].getId();
	...
	var ttimplieshtml = '<div id = "ttimpliesMain" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"> <!-- --> <div id="directions" class = "modal-header"> <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button> <h3 id="header">Build the truth table for the AND operator: </h3> <h4>P * Q</h4> </div> <!--The main modal body contains only the exercise.--> <div id="ttimplies" class = "modal-body"></div> <div id = "buttonDiv" class="modal-footer"> <!--This button allows the user to close the modal and return to reading the material. Work will not be saved.--> <button class="btn" data-dismiss="modal" aria-hidden="true">Close</button> <!--This button checks the user\'s work. Calls "ttimpliesupdateTable" function. ttimpliesupdateTable() is found in functions.js. --> <input type = "submit" class="btn btn-primary" value = "check it!" id = "ttimpliessubmit" onclick = "ttimpliesupdateTable();" /> </div> </div>';
	function writettimplies(){
		$('#widgethtml').html(ttimplieshtml);
		ttimpliesCall();
	}
