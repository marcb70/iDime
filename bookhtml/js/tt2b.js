	...
	var tt2bcirleArray = new Array();
	var tt2bcirle4Array = new Array();
	var tt2bcirle5Array = new Array();
	var tt2bcirle6Array = new Array();
	var tt2btrashArray = new Array();
...
function tt2bCall(){
	...
	*This file uses kineticJS to create each object.*/
	...
	var PimpliesR = new Kinetic.Text({
	  x: 160,
	  y: 15,
	  text: 'P -> R',
	  fontSize: 15,
	...
	/*creates an event each time a circle is clicked or tapped. Calls upon tt2bsetColor() function in functions.js
	*for devices with a mouse device, the click function is used. For touch devices such as mobile and tablet devices, the tap function is used.*/
	...
	function tt2bclearColumn(object){
		//console.log(object);
	...
	tt2baddToArray to add the object to its corresponding array and draws the layer to ensure color change.*/
	function tt2bsetColor(object)
	{
		...
		tt2baddToArray(object);//add the object with it's id and color to an array
		tableShapes.draw();//draw the layer to make the color change. 
	}

	//this function serves to add an object to its correct array.
	function tt2baddToArray(object){
		//console.log("tt2baddToArray information: " + object.getFill() + ", " + object.getId());
		if(!tt2bcirleArray.contains(object)){
				tt2bcirleArray.push(object);
		}
		var objectId = object.getId();

		if(objectId=="4x1"||objectId=="4x2"||objectId=="4x3"||objectId=="4x4"||objectId=="4x5"||objectId=="4x6"||objectId=="4x7"||objectId=="4x8"){
			if(!tt2bcirle4Array.contains(object)){
				//console.log("pushing to tt2bcirle4Array" + objectId + " " + object.getFill());
				tt2bcirle4Array.push(object);
			}
		}
		else if(objectId=="5x1"||objectId=="5x2"||objectId=="5x3"||objectId=="5x4"||objectId=="5x5"||objectId=="5x6"||objectId=="5x7"||objectId=="5x8"){
			if(!tt2bcirle5Array.contains(object)){
				//console.log("pushing to tt2bcirle5Array" + objectId + " " + object.getFill());
				tt2bcirle5Array.push(object);
			}
		}
		else if(objectId=="6x1"||objectId=="6x2"||objectId=="6x3"||objectId=="6x4"||objectId=="6x5"||objectId=="6x6"||objectId=="6x7"||objectId=="6x8"){
			if(!tt2bcirle6Array.contains(object)){
				//console.log("pushing to tt2bcirle6Array" + objectId + " " + object.getFill());
				tt2bcirle6Array.push(object);
			}
		}
		else{
			//console.log("pushing to tt2btrashArray" + objectId + " " + object.getFill());
			tt2btrashArray.push(object);
		}
...
function tt2bhint(){
	...
	function tt2bupdateTable()
	{
		...
		if(tt2bcirleArray.length<24){
			errors++;
		}
		else{

			for(var j=0; j<tt2bcirle4Array.length; j++){
				var color1 = tt2bcirle4Array[j].getFill();
				var object1 = tt2bcirle4Array[j].getId();
				//console.log("for loop 2 Object id and color: " + object1 + ", " + color1);
				if(tt2bcirle4Array.length<8){
					errors++;
				}
				else{
					if(!((object1 == "4x1" && color1 == "green")||(object1 == "4x2" && color1 == "red")
			 			|| (object1 == "4x3" && color1 == "green") ||  (object1 == "4x4" && color1=="red")||
			...
			for(var k=0; k<tt2bcirle5Array.length; k++){
				var color2 = tt2bcirle5Array[k].getFill();
				var object2 = tt2bcirle5Array[k].getId();
				//console.log("for loop 3 Object id and color: " + object2 + ", " + color2);
				if(tt2bcirle5Array.length<8){
					errors++;
				}
				else{
					if(!((object2 == "5x1" && color2 == "red")||(object2 == "5x2" && color2 == "red")
						...
						var temp2="The ~P * R column has errors.";
			...
			for(var l=0; l<tt2bcirle6Array.length; l++){
				var color3 = tt2bcirle6Array[l].getFill();
				var object3 = tt2bcirle6Array[l].getId();
				//console.log("for loop 3 Object id and color: " + object3 + ", " + color3);
				if(tt2bcirle6Array.length<8){
					errors++;
				}
				else{
		...
		if(errors==0){
			...
			if(tt2bcirle4Array.length<8||tt2bcirle5Array.length<8||tt2bcirle6Array.length<8||tt2bcirleArray.length<24)
			{
	...
	var tt2bhtml = '<div id = "tt2bMain" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true"> <!-- --> <div id="directions" class = "modal-header"> <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button> <h3 id="header">P, Q and R have been filled in for you. Now, build the truth table for this statement: </h3> <h4>(P -> R) # (~P * Q)</h4> </div> <!--The main modal body contains only the exercise.--> <div id="tt2b" class = "modal-body"></div> <div id = "buttonDiv" class="modal-footer"> <!--This button allows the user to close the modal and return to reading the material. Work will not be saved.--> <button class="btn" data-dismiss="modal" aria-hidden="true">Close</button> <!--This button checks the user\'s work. Calls "tt2bupdateTable" function. tt2bupdateTable() is found in functions.js. --> <input type = "submit" class="btn btn-primary" value = "check it!" id = "tt2bsubmit" onclick = "tt2bupdateTable();" /> <!--This button gives the user a hint Calls "hint" function. --> <input type = "submit" class="btn btn-danger" value = "Give me a Hint!" id = "tt2bhintme" onclick = "tt2bhint();" /> </div> </div>';
	function writett2b(){
	  $('#widgethtml').html(tt2bhtml);
	  tt2bCall();
